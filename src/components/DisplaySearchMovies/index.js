...
class DisplaySearchMovies extends Component {
  ...
  fetchNewMoviePage = () => {
    const {userEnteredInput, searchPageNum} = this.state
    fetch(
      `https://api.themoviedb.org/3/search/movie?api_key=a296c915c9f82c25cca95eab8568c3a2&language=en-US&query=${userEnteredInput}&page=${searchPageNum}`,
    )
